...
struct Cell2D {
    ...
    // Size = cell_block_size(). Layout matches local_angle_group_offset().
...
struct IterationStats {
    ...
    void iterate() { error_previous = final_error; }
};

struct CpuLUCache {
    // Per-thread scratch buffers for cell-matrix assembly, LU factorisation,
    // and back-solve.  Instead of storing one LU factor per cell (O(ncells)
    // memory), we store one reusable workspace per thread (O(nthreads)).
    //
    // Layout (flat, packed by thread index):
    //   thread_lu    [tid * cell_block_elems .. (tid+1)*cell_block_elems - 1]
    //   thread_pivots[tid * cell_block_size  .. (tid+1)*cell_block_size  - 1]
    //
    // The cell matrix is assembled into the thread slot, factored in-place,
    // and used to solve the RHS — all within a single call to solve_cells_cpu.
    std::vector<double> thread_lu;       // [num_threads * cell_block_elems]
    std::vector<int>    thread_pivots;   // [num_threads * cell_block_size]
    int  num_threads_cached = 0;         // thread count at last allocation
    bool valid              = false;     // true once buffers are allocated
};
...
inline int cell_id(int i, int j, int nx) { return j * nx + i; }
...
inline int global_offset(const Problem2D& problem, int cell, int group, int dir, int dof) {
    return cell * problem.cell_block_size()
         + local_angle_group_offset(problem, group, dir, dof);
}

inline int face_offset_west_east(const Problem2D& problem,
                                 int boundary_j, int group, int dir, int face_dof) {
    return (((boundary_j * problem.groups + group) * problem.num_dirs() + dir) * 4) + face_dof;
}

inline int face_offset_south_north(const Problem2D& problem,
                                   int boundary_i, int group, int dir, int face_dof) {
...
struct TransportOutputFiles2D {
    ...
    /// If true (default), scalar and angular flux are written to each VTK step.
    /// Set to false when the transport solver is embedded in TRT to reduce I/O.
    bool save_flux = true;
};
...
#endif // THEREFORE_TRANSPORT2D_HPP
